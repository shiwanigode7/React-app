/**TSX file for the SIR presentation styling */

import { LightTheme } from "@esko/cloud-ui-components/dist/esm";
import { GridDirection, makeStyles } from "@material-ui/core";
import { COLOR_GRAPHITE_1, COLOR_GRAPHITE_4, COLOR_GRAPHITE_5, COLOR_GREY_AVATAR, COLOR_WHITE, RISK_SCORE_GREEN, RISK_SCORE_ORANGE, RISK_SCORE_RED } from "../constant/Colors";

export const GRID_DIRECTION_COLUMN: GridDirection = "column";
export const GRID_DIRECTION_ROW: GridDirection = "row";

declare interface RiskScoreThemeProps {
    riskScoreValue: number;
}

declare interface OwnerAvatarThemeProps {
    ownerFirstName: string;
    ownerLastName: string;
}

export const SIRPresentationStyles = makeStyles(() => ({
    root: {
        height: "98%",
        minWidth: "1000px"
    },
...
export const CPDRiskScoreStyles = makeStyles(() => ({
    avatar: (themeInput: RiskScoreThemeProps) => ({
        width: "75px",
        height: "75px",
        fontSize: "48px",
        backgroundColor: (
            themeInput.riskScoreValue >= 4 && themeInput.riskScoreValue <= 6) ?
            RISK_SCORE_GREEN :
            (
                (themeInput.riskScoreValue >= 7 && themeInput.riskScoreValue <= 9) ?
                    RISK_SCORE_ORANGE : RISK_SCORE_RED
            )
    })
}));

/**Styles for the owner */
export const SIRPresentationOwnerStyles = makeStyles(() => ({
